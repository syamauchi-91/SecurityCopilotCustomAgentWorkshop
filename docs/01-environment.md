...
3. GitHub Copilot のチャットウインドウを開きます (**Ctrl + ALT + I ボタンで開きます**)
4. チャットウインドウに次のプロンプトを送って、LLM による問い合わせ、応答が返ることを確認しましょう。

```text
こんにちは。
現在の日時を除いて、あなた自身について3行で説明してください。
```

**期待結果:** Copilot が自分自身の説明を返します。
